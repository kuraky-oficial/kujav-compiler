...
impl Compiler {
    // CAMBIADO: Ahora detecta cualquier objeto (String o Arreglo)
    pub fn is_ref_expr(&self, expr: &Expr) -> bool {
        match expr {
            Expr::String(_) | Expr::ArrayLiteral(_) => true,
            Expr::Identifier(n) => {
                if let Some(t) = self.variable_types.get(n) {
                    // Si el tipo guardado empieza con [ o L, es una referencia
                    t.starts_with('L') || t.starts_with('[')
                } else { false }
            },
            Expr::ArrayAccess(_, _) => false, // Acceder a un arreglo de Ints devuelve un Int
        ...
        match expr {
            ...
            Expr::Identifier(n) => {
                if let Some(&slot) = self.variables.get(&n) {
                    ...
                    self.current_bytecode.push(if is_ref { 0x19 } else { 0x15 }); // aload o iload
                    self.current_bytecode.push(slot);
                }
            }
            Expr::ArrayLiteral(elems) => {
                self.current_bytecode.push(0x10); 
                self.current_bytecode.push(elems.len() as u8);
                self.current_bytecode.push(0xBC); 
                self.current_bytecode.push(10); // T_INT
                for (i, e) in elems.into_iter().enumerate() {
                    self.current_bytecode.push(0x59); // dup
                    self.current_bytecode.push(0x10); 
                    self.current_bytecode.push(i as u8);
                    self.compile_expression(e);
                    self.current_bytecode.push(0x4F); // iastore (para arreglos de int)
                }
            }
            Expr::ArrayAccess(name, idx) => {
                if let Some(&slot) = self.variables.get(&name) {
                    self.current_bytecode.push(0x19); // aload (cargar el arreglo)
                    self.current_bytecode.push(slot); 
                    self.compile_expression(*idx);
                    self.current_bytecode.push(0x2E); // iaload (extraer el entero)
                }
            }
            Expr::Binary(l, op, r) => {
                if op == "+" && (self.is_ref_expr(&l) || self.is_ref_expr(&r)) {
                    // --- Lógica StringBuilder (aplanada para evitar E0499) ---
                    ...
                    self.current_bytecode.push(0xB6); self.current_bytecode.extend_from_slice(&m_app_l.to_be_bytes());
            ...
            Expr::Call(name, args) => {
                ...
                let m_u = self.cp.add_utf8(&name);
                let s_u = self.cp.add_utf8("()I");
                let nt = self.cp.add_name_and_type(m_u, s_u);
