...
fn main() -> std::io::Result<()> {
    ...
    // 1. Registramos constantes necesarias antes de compilar
    ...
    let main_n = kujav.cp.add_utf8("main");
    let main_s = kujav.cp.add_utf8("([Ljava/lang/String;)V");
    let code_a = kujav.cp.add_utf8("Code");

    // 2. Compilar (esto llenará kujav.methods)
    for stmt in ast { kujav.compile_statement(stmt); }
    kujav.current_bytecode.push(0xB1); 

    // 3. Generar archivo .class
    let mut file = fs::File::create("Salida.class")?;
    file.write_all(&[0xCA, 0xFE, 0xBA, 0xBE, 0x00, 0x00, 0x00, 0x31])?; // Java 5
    file.write_all(&kujav.cp.to_bytes())?; // Pozo de constantes final
    ...
    file.write_all(&main_n.to_be_bytes())?; file.write_all(&main_s.to_be_bytes())?;
    file.write_all(&[0x00, 0x01])?; file.write_all(&code_a.to_be_bytes())?;
